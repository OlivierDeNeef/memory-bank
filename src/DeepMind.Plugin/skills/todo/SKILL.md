...
Use the **Agent tool** to spawn a single subagent. Pass it a prompt with the parsed task details and the instructions below. Do NOT call any `mcp__deepmind__*` tools yourself.
...
**Check duplicates:** Call `mcp__deepmind__recall(query: "<task summary>", category: "todos", limit: 3)`. If a very similar open todo exists, return it so the main agent can ask the user.

**Store:** Call `mcp__deepmind__remember` with: content formatted as "TODO: <task>\nStatus: open\nDue: <date or 'none'>\nCreated: <today>", summary: "<concise task summary>", category: "todos", tags: "<tags, always include 'todo'>", type: "fact", priority: <1-5>, source: "conversation".
...
Use the **Agent tool**. Do NOT call any `mcp__deepmind__*` tools yourself.

**Subagent prompt:** Call `mcp__deepmind__recall(query: "TODO status open", category: "todos", limit: 50, sort: "priority")`. Return each todo's title/task, due date, priority label, tags, and status. Exclude memory IDs, similarity scores, chunk info, raw JSON.
...
**Subagent prompt:** Call `mcp__deepmind__recall(query: "<search term>", category: "todos", limit: 5)`. Return each match's memory ID, task title, due date, and priority. Exclude raw JSON and chunk metadata.
...
After user confirms, use the **Agent tool**. Do NOT call any `mcp__deepmind__*` tools yourself.

**Subagent prompt:** Call `mcp__deepmind__update_memory(id: "<id>", content: "<content with Status changed to 'done'>")` then `mcp__deepmind__archive(id: "<id>")`. Return confirmation with the task title.
