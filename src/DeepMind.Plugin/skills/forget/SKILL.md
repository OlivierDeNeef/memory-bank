...
Use the **Agent tool** to spawn a subagent that finds the deletion target(s). Do NOT call any `mcp__deepmind__*` tools yourself.

**Subagent prompt:** Search for memories matching the user's query or ID. If a UUID was provided, call `mcp__deepmind__get_memory(id: "<id>")`. If a search query, call `mcp__deepmind__recall(query: "<query>", limit: 10)`. Return for each match: the memory ID, title/summary, category, tags, priority label, revision number, and created date. Exclude raw JSON, chunk metadata, embedding details.
...
After user confirms, use the **Agent tool** to spawn a subagent that performs the deletion. Do NOT call any `mcp__deepmind__*` tools yourself.

**Subagent prompt:** Delete the confirmed memory/memories. For single delete: call `mcp__deepmind__forget(id: "<memory-id>")`. For bulk delete: call `mcp__deepmind__bulk_forget(category: "<if set>", tag: "<if set>")`. Return the count of deleted memories and their titles.
