...
Use the **Agent tool** to spawn a single subagent. Pass it a prompt with the scope and the instructions below. Do NOT call any `mcp__deepmind__*` tools yourself.
...
**Full overview:** Call `mcp__deepmind__memory_stats()` and `mcp__deepmind__health_check()` in parallel.

**Health only:** Call `mcp__deepmind__health_check()`.

**Categories:** Call `mcp__deepmind__list_categories()`.

**Tags:** Call `mcp__deepmind__list_tags(sort: "count", limit: 50)`.
