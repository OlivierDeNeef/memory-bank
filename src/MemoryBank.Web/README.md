...
3D force-graph viewer for the MemoryBank memory store. Read-only.

## Stack

- **Backend**: ASP.NET Core (.NET 10) minimal API, references `MemoryBank.Core`
...
```bash
dotnet publish src/MemoryBank.Web -c Release
```

The MSBuild pipeline runs `npm ci && npm run build` and ships the static bundle in `wwwroot`. A single `dotnet MemoryBank.Web.dll` serves both the API and the SPA on `http://localhost:5174`.
