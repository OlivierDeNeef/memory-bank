...
public class MemoryStoreTests : IDisposable
{
    ...
    public MemoryStoreTests()
    {
        ...
        _config.Database.Path = Path.Combine(Path.GetTempPath(), $"memorybank_test_{Guid.NewGuid()}.db");
...
public class SearchEngineTests : IDisposable
{
    ...
    {
        _config = new MemoryBankConfiguration();
        _config.Database.Path = Path.Combine(Path.GetTempPath(), $"memorybank_search_test_{Guid.NewGuid()}.db");
        _db = new MemoryBankDb(_config, NullLogger<MemoryBankDb>.Instance);
        _store = new MemoryStore(_db, _config, NullLogger<MemoryStore>.Instance);
        _chunking = new ChunkingService(_config.Embedding);
...
public class BackupServiceTests : IDisposable
{
    ...
    {
        _config = new MemoryBankConfiguration();
        var tempDir = Path.Combine(Path.GetTempPath(), $"memorybank_backup_test_{Guid.NewGuid()}");
        _config.Database.Path = Path.Combine(tempDir, "memorybank.db");
    ...
    public void Retention_Enforced()
    {
        ...
        var files = Directory.GetFiles(_config.Backup.Path, "memorybank_backup_*.db");
