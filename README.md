...
| **Node.js 20+** | Only for the graph viewer's SPA build (`DeepMind.Web/ClientApp`) |
...
2. Place it at:
   ```
   src/DeepMind.Core/Embeddings/Models/model.onnx
   ```
...
Edit `src/DeepMind.Server/appsettings.json` (or pass `DeepMind__Embedding__*` env vars):
...
> **Switching models with existing data**: embeddings stored at the old dimension count are incompatible with the new model. Delete `~/.deepmind/deepmind.db` (or re-embed via the `reembed_all` MCP tool) after switching.

---

## Running it

There are three ways to run DeepMind. Pick based on what you want to do.
...
The database persists in the `deepmind-data` Docker volume across restarts. To wipe it:
```bash
docker compose down -v
```

**Note**: the embedding model needs to be at `src/DeepMind.Core/Embeddings/Models/model.onnx` *before* you build the Docker image — the `CopyToOutputDirectory` in the csproj bakes it into the image. If it's missing, vector search will be silently disabled inside the container.
...
```bash
dotnet run --project src/DeepMind.Server
```

**MCP server (HTTP, port 6868):**
```bash
dotnet run --project src/DeepMind.Server -- --http
```
...
```bash
dotnet publish src/DeepMind.Web -c Release -o publish/web
dotnet ./publish/web/DeepMind.Web.dll
# → http://localhost:5174
```

### Option C — Use it with Claude Code

DeepMind's primary purpose is to be an MCP server for AI assistants. For Claude Code there are **two things to register**:

1. The **MCP server** — so Claude can actually call DeepMind's tools
2. The **plugin** at `src/DeepMind.Plugin/` — provides slash commands (`/remember`, `/recall`, `/search`, `/stats`, `/todo`, `/forget`) and a PreToolUse hook that ensures all memory access goes through the skill layer rather than raw MCP tool calls
...
claude mcp add --transport stdio deepmind -- dotnet run --project ./src/DeepMind.Server
```

For faster startup (no JIT on each launch), publish first:
```bash
dotnet publish src/DeepMind.Server -c Release -o publish/server

claude mcp add --transport stdio deepmind -- dotnet ./publish/server/DeepMind.Server.dll
```

**HTTP (when running via Docker)** — the `deepmind` container already exposes MCP on `:6868`:
```bash
claude mcp add --transport http deepmind http://localhost:6868/mcp
```
...
```bash
claude --plugin-dir ./src/DeepMind.Plugin
```

**For permanent install** — register the local plugin directory as a marketplace and install from it:
```
/plugin marketplace add ./src/DeepMind.Plugin
/plugin install deepmind@deepmind-local
/reload-plugins
```

After the reload, `/remember`, `/recall`, `/search`, `/stats`, `/todo`, and `/forget` are available as slash commands. The plugin's PreToolUse hook at `scripts/block-direct-mcp.sh` intercepts any direct `mcp__deepmind__*` calls and nudges Claude to go through the skills instead.

---

## Configuration

Defaults live in `src/DeepMind.Core/Configuration/DeepMindConfiguration.cs` and can be overridden via `appsettings.json` or environment variables with the `DeepMind__` prefix.

| Setting | Default | Env var |
|---|---|---|
| Database path | `~/.deepmind/deepmind.db` | `DeepMind__Database__Path` |
| Backup path | `~/.deepmind/backups` | `DeepMind__Backup__Path` |
| Embedding model path | `~/.deepmind/models/nomic-embed-text-v1.5.onnx` | `DeepMind__Embedding__ModelPath` |
| Embedding dimensions | 768 | `DeepMind__Embedding__Dimensions` |
| Log file | `~/.deepmind/logs/deepmind.log` | `DeepMind__Logging__FilePath` |

The model path is also checked relative to the running assembly — which is why the `Models/` subfolder in `DeepMind.Core` works automatically once you drop the `.onnx` file in.

In the Docker container these defaults are overridden to `/data/deepmind/*` so they land on the mounted volume.
...
```
src/
├── DeepMind.Core/           Shared library: storage, search, embeddings, config
...
└── DeepMind.Plugin/         Claude Code plugin with skills (remember, recall, search, ...)

tests/
└── DeepMind.Tests/          Smoke tests

Dockerfile                   Image for the MCP server (HTTP transport on :6868)
Dockerfile.web               Multi-stage image for the graph viewer (:5174)
docker-compose.yml           Both services sharing a deepmind-data volume
```
...
See `src/DeepMind.Web/README.md` for the web-specific API and dev details.
...
→ Check for a leftover `DeepMind.Web` dev process on `127.0.0.1:5174` — Windows will prefer it over Docker's `0.0.0.0:5174` forward. Kill it and reload.
...
→ Either a corrupt DB file or a schema mismatch from a previous version. Back up `~/.deepmind/deepmind.db` and delete it to start fresh.
